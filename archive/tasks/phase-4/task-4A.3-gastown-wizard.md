...
## What to Build
Backend commands for the Gastown setup wizard: detect CLI, check workspace, add rig, run doctor. All via visible terminal commands.

## Setup Steps
1. Check gt CLI: `which gt` → found/not found
2. Check workspace: does ~/gt/ exist?
3. Add rig: `gt rig add {name}` in visible terminal
4. Health check: `gt doctor` in visible terminal
5. Report status back to frontend

## Deliverables
1. `gastown/setup_wizard.rs` — GastownSetup struct with check_cli(), check_workspace(), add_rig(), run_doctor()
2. Each step returns structured result (success/failure + details)
3. All gt commands execute in visible PTY panes
...
## Acceptance Test
Run setup check → correctly detects gt CLI presence, workspace existence, rig status. Run doctor → output visible in terminal pane.
